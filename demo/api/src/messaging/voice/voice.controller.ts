...
import type { IVoiceArgs } from 'tnzapi-ts';
...
export class VoiceController {
    ...
    @Post('send')
    async send(@Body() body: SendVoiceDto, @Req() req: Request, @Res() res: Response): Promise<void> {
    ...
    async abort(@Param('messageId') messageId: string, @Req() req: Request, @Res() res: Response): Promise<void> {
        const client = this.tnzClient.getClient(req);
        const result = await client.Actions.Abort.SendRequest({ MessageID: messageId, Channel: 'voice' });
        respondWithResult(res, result);
    }
