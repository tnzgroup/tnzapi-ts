...
function makeApi(authToken = AUTH) {
    ...
    return { api: new RCSApi({ URL: BASE_URL, AuthToken: authToken, httpClient }), httpClient };
}

describe('RCSApi — validation', () => {
    ...
    it('accepts TemplateID in place of Message', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-tmpl' });
        const result = await api.SendMessage({
    ...
    it('accepts WebhookCallbackFormat POST and GET', async () => {
        for (const fmt of [WebhookCallbackFormat.POST, WebhookCallbackFormat.GET]) {
            const { api, httpClient } = makeApi();
            httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-fmt' });
            const result = await api.SendMessage({
                Message: 'Hi',
    ...
    it('sends FallbackMode in payload', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs001' });
    ...
    it('calls httpClient.post to /rcs for a valid request', async () => {
        const { api, httpClient } = makeApi();
...
describe('RCSApi — AddRecipient', () => {
    ...
    it('accumulates destinations added via AddRecipient', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-ar' });
...
describe('RCSApi — single-destination shorthand', () => {

    it('accepts a single ToNumber shorthand instead of Destinations', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-short1' });
        const result = await api.SendMessage({
            Message: 'Hi',
    ...
    it('splits a comma-separated ToNumber shorthand into multiple destinations', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-short2' });
        await api.SendMessage({
            Message: 'Hi',
    ...
    it('accepts GroupID and ContactID shorthand fields', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-short3' });
        await api.SendMessage({
            Message: 'Hi',
    ...
    it('merges the ToNumber shorthand additively with an explicit Destinations array', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-short4' });
        await api.SendMessage({
            Message: 'Hi',
            ToNumber: '+64211111111',
    ...
    it('never leaks the raw shorthand fields into the request payload', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'rcs-short5' });
        await api.SendMessage({
            Message: 'Hi',
            ToNumber: '+64211111111',
            GroupID: '4000000b-f002-4007-b00a-c00000000005',
