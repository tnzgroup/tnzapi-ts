...
function makeApi(authToken = AUTH) {
    ...
    return { api: new VoiceApi({ URL: BASE_URL, AuthToken: authToken, httpClient }), httpClient };
}

describe('VoiceApi — validation', () => {
    ...
    it('accepts TemplateID in place of MessageToPeople', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice-tmpl' });
        const result = await api.SendMessage({
    ...
    it('accepts WebhookCallbackFormat POST and GET', async () => {
        for (const fmt of [WebhookCallbackFormat.POST, WebhookCallbackFormat.GET]) {
            const { api, httpClient } = makeApi();
            httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice-fmt' });
            const result = await api.SendMessage({
                MessageToPeople: 'Hello',
                WebhookCallbackURL: 'https://example.com/hook',
    ...
    it('sends MessageToAnswerPhones and AnswerPhoneMode in payload', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice001' });
    ...
    it('sends Keypads with numeric Tone in payload', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice002' });
        await api.SendMessage({
    ...
    it('sends KeypadOptionRequired and CallRoute fields in payload', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice003' });
        await api.SendMessage({
    ...
    it('calls httpClient.post to /voice for a valid request', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice004' });
        const result = await api.SendMessage({
    ...
    it('returns Error when VoiceFiles contains a non-existent file path', async () => {
        const { api, httpClient } = makeApi();
        const result = await api.SendMessage({
            VoiceFiles: [{ Name: 'MessageToPeople', File: '/non-existent/audio.wav' }],
            Destinations: [{ MainPhone: '+6492345678' }],
        });
        expect(result.Result).toBe('Error');
    ...
    it('returns Error when Keypads contain a non-existent audio file', async () => {
        const { api, httpClient } = makeApi();
        const result = await api.SendMessage({
            MessageToPeople: 'Press 1.',
            Destinations: [{ MainPhone: '+6492345678' }],
...
describe('VoiceApi — single-destination shorthand', () => {

    it('accepts a single ToNumber shorthand, which resolves to a MainPhone destination', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice-short1' });
        const result = await api.SendMessage({
            MessageToPeople: 'Hi',
            ToNumber: '+6492345678',
        });
    ...
    it('splits a comma-separated ToNumber shorthand into multiple MainPhone destinations', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice-short2' });
        await api.SendMessage({
            MessageToPeople: 'Hi',
    ...
    it('accepts GroupID and ContactID shorthand fields', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice-short3' });
        await api.SendMessage({
            MessageToPeople: 'Hi',
    ...
    it('merges the ToNumber shorthand additively with an explicit Destinations array', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice-short4' });
        await api.SendMessage({
            MessageToPeople: 'Hi',
            ToNumber: '+6492345678',
    ...
    it('never leaks the raw shorthand fields into the request payload', async () => {
        const { api, httpClient } = makeApi();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success', MessageID: 'voice-short5' });
        await api.SendMessage({
            MessageToPeople: 'Hi',
            ToNumber: '+6492345678',
            GroupID: '4000000b-f002-4007-b00a-c00000000005',
