...
describe('GroupListApi', () => {
    ...
    it('rejects when AuthToken is missing', async () => {
        ...
        const api = new GroupListApi({ URL: BASE_URL, AuthToken: '', httpClient });
    ...
    it('rejects when RecordsPerPage is not a number', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupListApi({ URL: BASE_URL, AuthToken: AUTH, httpClient });
        const result = await api.Run({ RecordsPerPage: 'ten' as unknown as number });
        expect(result.Result).toBe('Error');
        expect((result as ErrorResponseDTO).ErrorMessage[0]).toMatch(/RecordPerPage must be a number/i);
        expect(httpClient.get).not.toHaveBeenCalled();
    });
    ...
    it('rejects when Page is not a number', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupListApi({ URL: BASE_URL, AuthToken: AUTH, httpClient });
        const result = await api.Run({ Page: 'first' as unknown as number });
        expect(result.Result).toBe('Error');
        expect((result as ErrorResponseDTO).ErrorMessage[0]).toMatch(/Page must be a number/i);
        expect(httpClient.get).not.toHaveBeenCalled();
    });
...
describe('GroupCreateApi', () => {
    ...
    it('rejects when AuthToken is missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupCreateApi({ URL: BASE_URL, AuthToken: '', httpClient });
    ...
    it('rejects when GroupName is missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupCreateApi({ URL: BASE_URL, AuthToken: AUTH, httpClient });
        const result = await api.Run({});
        expect(result.Result).toBe('Error');
        expect((result as ErrorResponseDTO).ErrorMessage[0]).toMatch(/GroupName/i);
        expect(httpClient.post).not.toHaveBeenCalled();
    });
    ...
    it('never leaks the constructor-only URL/AuthToken/httpClient into the very first Run payload', async () => {
        const httpClient = makeMockHttpClient();
        httpClient.post.mockResolvedValueOnce({ Result: 'Success' });
        const api = new GroupCreateApi({ URL: BASE_URL, AuthToken: 'super-secret-token', httpClient });
        await api.Run({ GroupName: 'My Group' });
...
describe('GroupDetailApi', () => {
    ...
    it('rejects when AuthToken is missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupDetailApi({ URL: BASE_URL, AuthToken: '', httpClient });
    ...
    it('rejects when both GroupID and GroupCode are missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupDetailApi({ URL: BASE_URL, AuthToken: AUTH, httpClient });
        const result = await api.Run({});
        expect(result.Result).toBe('Error');
        expect((result as ErrorResponseDTO).ErrorMessage[0]).toMatch(/GroupID or GroupCode/i);
        expect(httpClient.get).not.toHaveBeenCalled();
    });
    ...
    it('percent-encodes a GroupCode containing reserved URL characters instead of splicing it into the path raw', async () => {
        const httpClient = makeMockHttpClient();
        httpClient.get.mockResolvedValueOnce({ Result: 'Success' });
        const api = new GroupDetailApi({ URL: BASE_URL, AuthToken: AUTH, httpClient });
        await api.Run({ GroupCode: 'a/b?c#d' });
        const [url] = httpClient.get.mock.calls[0];
...
describe('GroupUpdateApi', () => {
    ...
    it('rejects when AuthToken is missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupUpdateApi({ URL: BASE_URL, AuthToken: '', httpClient });
    ...
    it('rejects when both GroupID and GroupCode are missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupUpdateApi({ URL: BASE_URL, AuthToken: AUTH, httpClient });
...
describe('GroupDeleteApi', () => {
    ...
    it('rejects when AuthToken is missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupDeleteApi({ URL: BASE_URL, AuthToken: '', httpClient });
        const result = await api.Run({ GroupID: 'GRP001' });
        expect(result.Result).toBe('Error');
        expect((result as ErrorResponseDTO).ErrorMessage[0]).toMatch(/AuthToken/i);
    ...
    it('rejects when both GroupID and GroupCode are missing', async () => {
        const httpClient = makeMockHttpClient();
        const api = new GroupDeleteApi({ URL: BASE_URL, AuthToken: AUTH, httpClient });
        const result = await api.Run({});
        expect(result.Result).toBe('Error');
        expect((result as ErrorResponseDTO).ErrorMessage[0]).toMatch(/GroupID or GroupCode/i);
        expect(httpClient.delete).not.toHaveBeenCalled();
